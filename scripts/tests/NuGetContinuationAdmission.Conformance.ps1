...
function New-Request([int]$ContentStatus,[AllowNull()][byte[]]$Bytes=$null,[bool]$ThrowContent=$false){
    ...
    return {param($request);$script:calls++;if($script:calls-eq 1){return [pscustomobject]@{StatusCode=200;Content=$serviceIndexContent;Bytes=$null}};if($ThrowContent){throw 'transport failed'};[pscustomobject]@{StatusCode=$ContentStatus;Content=$null;Bytes=$Bytes}}.GetNewClosure()
}
...
$results=@()
$results+=Invoke-Case 'A01' '404 is authoritative absence and Admissible' {$p=New-Package;$r=Observe $p (New-Decision) (New-Request 404);Assert-Eq 'Admissible' $r.State 'state';Assert-Eq 'AuthoritativeAbsent' $r.Reason 'reason';Assert-Eq '404' ([string]$r.StatusCode) 'status';Assert-Eq '2' ([string]$script:calls) 'request count'}
$results+=Invoke-Case 'A02' 'exact observation uses GET package-content URI' {$seen=@();$serviceIndexContent=$serviceJson;$req={param($r)$seen+=$r;if($seen.Count-eq 1){[pscustomobject]@{StatusCode=200;Content=$serviceIndexContent;Bytes=$null}}else{[pscustomobject]@{StatusCode=404;Content=$null;Bytes=$null}}}.GetNewClosure();$null=Observe (New-Package) (New-Decision) $req;Assert-Eq 'GET' $seen[1].Method 'method';Assert-Eq ($base+'pkg.d/1.0.4-test.1/pkg.d.1.0.4-test.1.nupkg') $seen[1].Uri 'content URI'}
$results+=Invoke-Case 'A03' '200 equal bytes are BlockedEquivalent' {$bytes=[Text.Encoding]::UTF8.GetBytes('same');$hash=Get-Hash $bytes;$r=Observe (New-Package -Hash $hash) (New-Decision -Hash $hash) (New-Request 200 $bytes);Assert-Eq 'Blocked' $r.State 'state';Assert-Eq 'EquivalentPackagePresent' $r.Reason 'reason';Assert-Eq $hash $r.RemoteSha256 'remote hash'}
$results+=Invoke-Case 'A04' '200 different bytes are BlockedDifferent' {$bytes=[Text.Encoding]::UTF8.GetBytes('remote');$r=Observe (New-Package) (New-Decision) (New-Request 200 $bytes);Assert-Eq 'Blocked' $r.State 'state';Assert-Eq 'DifferentPackagePresent' $r.Reason 'reason'}
$results+=Invoke-Case 'A05' '200 without complete bytes is Indeterminate' {$r=Observe (New-Package) (New-Decision) (New-Request 200 $null);Assert-Eq 'Indeterminate' $r.State 'state';Assert-Eq 'ContentUnreadable' $r.Reason 'reason'}
