...
function New-Request([int]$ContentStatus,[AllowNull()][byte[]]$Bytes=$null,[bool]$ThrowContent=$false){
    $script:calls=0
    return {param($request);$script:calls++;if($script:calls-eq 1){return [pscustomobject]@{StatusCode=200;Content=$serviceJson;Bytes=$null}};if($ThrowContent){throw 'transport failed'};[pscustomobject]@{StatusCode=$ContentStatus;Content=$null;Bytes=$Bytes}}.GetNewClosure()
}
...
$results+=Invoke-Case 'A02' 'exact observation uses GET package-content URI' {$seen=@();$req={param($r)$seen+=$r;if($seen.Count-eq 1){[pscustomobject]@{StatusCode=200;Content=$serviceJson;Bytes=$null}}else{[pscustomobject]@{StatusCode=404;Content=$null;Bytes=$null}}}.GetNewClosure();$null=Observe (New-Package) (New-Decision) $req;Assert-Eq 'GET' $seen[1].Method 'method';Assert-Eq ($base+'pkg.d/1.0.4-test.1/pkg.d.1.0.4-test.1.nupkg') $seen[1].Uri 'content URI'}
