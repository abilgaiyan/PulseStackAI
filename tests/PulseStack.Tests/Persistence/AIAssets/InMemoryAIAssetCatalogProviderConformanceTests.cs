    ...
    : AIAssetCatalogProviderConformanceTests
{
    ...
    [Fact]
    public async Task FindLineageAsync_ShouldRejectSameLineageIdentityWithDifferentExactRecordUrn()
    {
        var catalogNamespace = new InMemoryAIAssetCatalogNamespace();
        var provider = new InMemoryAIAssetCatalogProvider(catalogNamespace);
        var id = AssetId.New();
        var lineageUrn = new AssetUrn($"urn:pulsestack:prompt:{Guid.NewGuid():N}");
        var conflictingUrn = new AssetUrn($"urn:pulsestack:prompt:{Guid.NewGuid():N}");
        var version1 = new AssetVersion("1.0");
        var version2 = new AssetVersion("2.0");
        var key1 = new AssetDefinitionKey(AssetType.Prompt, id, version1);
        var key2 = new AssetDefinitionKey(AssetType.Prompt, id, version2);
        ...
        lock (catalogNamespace.SyncRoot)
        {
            catalogNamespace.UrnByLineage.Add((AssetType.Prompt, id), lineageUrn);
            catalogNamespace.LineagesByUrn.Add(
                lineageUrn,
                new InMemoryCatalogLineageState(AssetType.Prompt, id, lineageUrn, version1));
            catalogNamespace.ExactRecords.Add(key1, new CatalogRecord(key1, lineageUrn));
            catalogNamespace.ExactRecords.Add(key2, new CatalogRecord(key2, conflictingUrn));
        }

        var act = async () => await provider.FindLineageAsync(lineageUrn);
    ...
    [Fact]
    public async Task FindLineageAsync_ShouldRejectSameUrnWithDifferentExactRecordLineageIdentity()
    {
        var catalogNamespace = new InMemoryAIAssetCatalogNamespace();
        var provider = new InMemoryAIAssetCatalogProvider(catalogNamespace);
        var promptId = AssetId.New();
        var agentId = AssetId.New();
        var urn = new AssetUrn($"urn:pulsestack:prompt:{Guid.NewGuid():N}");
        var version1 = new AssetVersion("1.0");
        var version2 = new AssetVersion("2.0");
        var promptKey = new AssetDefinitionKey(AssetType.Prompt, promptId, version1);
        var agentKey = new AssetDefinitionKey(AssetType.Agent, agentId, version2);

        lock (catalogNamespace.SyncRoot)
        {
            catalogNamespace.UrnByLineage.Add((AssetType.Prompt, promptId), urn);
            catalogNamespace.LineagesByUrn.Add(
                urn,
                new InMemoryCatalogLineageState(AssetType.Prompt, promptId, urn, version1));
            catalogNamespace.ExactRecords.Add(promptKey, new CatalogRecord(promptKey, urn));
            catalogNamespace.ExactRecords.Add(agentKey, new CatalogRecord(agentKey, urn));
        }
