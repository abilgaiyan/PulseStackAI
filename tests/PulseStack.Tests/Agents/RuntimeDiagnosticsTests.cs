...
{
    [Fact]
    public async Task ParallelPipeline_Should_Publish_Pipeline_Lifecycle_Events()
    {
        ...
        dispatcher.Events.Should().HaveCount(2);
        dispatcher.Events[0].Should().BeOfType<PipelineStartedEvent>();
        dispatcher.Events[1].Should().BeOfType<PipelineCompletedEvent>();

        var started = (PipelineStartedEvent)dispatcher.Events[0];
        var completed = (PipelineCompletedEvent)dispatcher.Events[1];
